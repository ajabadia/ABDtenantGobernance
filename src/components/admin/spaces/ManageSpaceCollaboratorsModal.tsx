...
interface ManageSpaceCollaboratorsModalProps {
  tenantId: string;
...
export function ManageSpaceCollaboratorsModal({
  tenantId,
  space,
  isOpen,
  onClose,
  onSuccess,
}: ManageSpaceCollaboratorsModalProps) {
  ...
  const matchedUsers = search.length >= 2 
    ? users.filter(u => !isAlreadyAdded(u._id) && (`${u.name} ${u.surname} ${u.email}`.toLowerCase().includes(search.toLowerCase()))).slice(0, 3)
    : [];
    
  const matchedGroups = search.length >= 2
    ? groups.filter(g => !isAlreadyAdded(g._id!.toString()) && g.name.toLowerCase().includes(search.toLowerCase())).slice(0, 3)
    : [];
  ...
  const handleRemove = (subjectId: string) => {
    setCollaborators(prev => prev.filter(c => c.subjectId !== subjectId));
  };

  const handleRoleChange = (subjectId: string, role: 'VIEWER' | 'EDITOR' | 'ADMIN') => {
    setCollaborators(prev => prev.map(c => c.subjectId === subjectId ? { ...c, role } : c));
  };

  const handlePropagatesChange = (subjectId: string, propagates: boolean) => {
    setCollaborators(prev => prev.map(c => c.subjectId === subjectId ? { ...c, propagates } : c));
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || 'Error saving collaborators');
      }
  ...
  const resolveSubjectName = (c: CollaboratorInput) => {
    if (c.subjectType === 'USER') {
      const u = users.find(x => x._id === c.subjectId);
      return u ? `${u.name} ${u.surname} (${u.email})` : 'Usuario Desconocido';
    } else {
      const g = groups.find(x => x._id!.toString() === c.subjectId);
      return g ? g.name : 'Grupo Desconocido';
    }
  };
      ...
      <div className="relative w-full max-w-2xl bg-card border border-border shadow-2xl p-8 rounded-none animate-in fade-in slide-in-from-bottom-4 max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between mb-6 pb-2 border-b border-border shrink-0">
          <h2 className="text-lg font-black uppercase italic tracking-tight text-foreground flex items-center gap-2">
            ...
            {t('manage_collaborators', { defaultMessage: 'Gobernanza del Espacio' })}
          </h2>
        ...
        <p className="font-mono text-[10px] text-muted-foreground mb-4 uppercase shrink-0">
          {t('space_label', { defaultMessage: 'Espacio:' })} <span className="text-primary font-bold">{space.name}</span> <span className="opacity-50">({space.materializedPath})</span>
        </p>

        {loading ? (
           <div className="p-8 text-center font-mono text-[10px] text-muted-foreground uppercase flex-1">
             {t('loading_tenant_data', { defaultMessage: 'CARGANDO DATOS DEL TENANT...' })}
           </div>
        ) : (
          <div className="flex flex-col gap-6 flex-1 overflow-hidden">
            ...
            <div className="flex flex-col gap-2 shrink-0 relative">
              <label className="text-[9px] font-mono font-black text-muted-foreground uppercase tracking-widest">
                {t('add_user_group', { defaultMessage: 'Añadir Usuario o Grupo' })}
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={t('search_placeholder', { defaultMessage: 'Buscar por nombre, email o grupo...' })}
                ...
                <div className="absolute top-full left-0 right-0 z-10 mt-1 bg-card border border-border shadow-lg">
                  {matchedGroups.map(g => (
                    <button
                      key={g._id!.toString()}
                      type="button"
                      aria-label={`Añadir grupo ${g.name}`}
                      onClick={() => handleAdd(g._id!.toString(), 'GROUP')}
                      className="w-full flex items-center justify-between px-4 py-2 hover:bg-primary/[0.05] border-b border-border/40 text-left transition-colors"
                    >
                      <div className="flex items-center gap-2">
                        <Shield size={12} className="text-amber-500" />
                        <span className="font-sans text-xs font-bold text-foreground">{g.name}</span>
                      </div>
                  ...
                  {matchedUsers.map(u => (
                    <button
                      key={u._id}
                      type="button"
                      aria-label={`Añadir usuario ${u.name}`}
                      onClick={() => handleAdd(u._id, 'USER')}
                      className="w-full flex items-center justify-between px-4 py-2 hover:bg-primary/[0.05] border-b border-border/40 text-left transition-colors"
                    >
                      <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
               <span className="text-[9px] font-mono font-black text-muted-foreground uppercase tracking-widest mb-2 shrink-0">
                 {t('assigned_collaborators', { defaultMessage: 'Colaboradores Asignados' })} ({collaborators.length})
               </span>
               <div className="border border-border divide-y divide-border/40 overflow-y-auto flex-1">
                 {collaborators.length === 0 ? (
                    <div className="p-4 text-center font-mono text-[10px] text-muted-foreground uppercase">
                      {t('no_explicit_collaborators', { defaultMessage: 'Sin colaboradores explícitos' })}
                    </div>
                 ) : (
                    collaborators.map(c => (
                      <div key={c.subjectId} className="flex items-center justify-between px-4 py-3 hover:bg-primary/[0.02]">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="font-sans text-xs font-bold text-foreground">{resolveSubjectName(c)}</span>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="flex items-center gap-2">
                            <label className="text-[9px] font-mono text-muted-foreground uppercase" title={t('heritage_title', { defaultMessage: 'Herencia a sub-espacios' })}>{t('heritage', { defaultMessage: 'Herencia' })}</label>
                            <input 
                              type="checkbox" 
                          ...
                          >
                            <option value="VIEWER">{t('role_viewer', { defaultMessage: 'VIEWER' })}</option>
                            <option value="EDITOR">{t('role_editor', { defaultMessage: 'EDITOR' })}</option>
                            <option value="ADMIN">{t('role_admin', { defaultMessage: 'ADMIN' })}</option>
                          </select>
                          <button
                            type="button"
        ...
        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-border shrink-0">
          ...
          <button
            type="button"
            aria-label={saving ? t('saving', { defaultMessage: 'Guardando...' }) : t('save', { defaultMessage: 'Guardar' })}
            onClick={handleSave}
          ...
          >
            {saving ? t('saving', { defaultMessage: 'Guardando...' }) : t('save', { defaultMessage: 'Guardar' })}
          </button>
        </div>
      </div>
    </div>
